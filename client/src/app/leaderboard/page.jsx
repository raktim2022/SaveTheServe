...
function LeaderRow({ rank, name, primary, secondary, delay }) {
  return (
    <div
      className="flex items-center gap-4 p-4 rounded-xl border border-gray-100 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm hover:shadow-md transition-shadow"
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-gray-500 dark:text-slate-400 truncate">{secondary}</p>
      </div>
      ...
      <main className="min-h-screen bg-gradient-to-b from-green-50/60 via-white to-white pt-24 pb-20">
        <div className="max-w-2xl mx-auto px-4 space-y-8">
          ...
          <div className="text-center space-y-2">
            ...
            <h1 className="text-3xl font-extrabold text-gray-900 dark:text-white">Impact Leaderboard</h1>
            <p className="text-gray-500 dark:text-slate-400">Celebrating the organizations rescuing the most food</p>
          </div>
            ...
            {TABS.map((t) => (
              <button
                ...
                className={`flex-1 py-2 text-sm font-medium rounded-lg transition-colors ${
                  activeTab === t.key ? 'bg-white dark:bg-slate-800 shadow-sm text-gray-900 dark:text-white' : 'text-gray-500 dark:text-slate-400 hover:text-gray-700 dark:text-slate-200'
                }`}
              >
                {t.label}
              </button>
